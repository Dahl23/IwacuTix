...
export const apiUserToUser = (apiUser: ApiUser, currentUser?: User, apiBaseUrl?: string): User => ({
  ...
  email_verifie: apiUser.email_verifie,
...
export const apiTicketToPurchased = (
  ...
  return {
    ...
    status: apiTicket.statut === 'UTILISE' ? 'utilise' : apiTicket.statut === 'ANNULE' ? 'annule' : 'valide',
    paymentMethod,
    ...
    recipientHasNoPhone: Boolean(apiTicket.destinataire_nom) && !apiTicket.destinataire_telephone,
  };
};
